...
BOOST_AUTO_TEST_SUITE(when_created)
		...
		BOOST_AUTO_TEST_CASE(end_and_begin_iterators_as_equal)
		{
	...
	BOOST_FIXTURE_TEST_SUITE(after_filled_the_string_list, FilledStringList)
		...
		{
			CMyList<std::string>::iterator end = list.end();
		};
		...
		BOOST_FIXTURE_TEST_SUITE(can_insert_data_in_iterator_pos, FilledStringList)
			...
			{
				BOOST_REQUIRE_THROW(list.Insert(CMyList<std::string>::iterator(), "crash!"), std::invalid_argument);
			}
		BOOST_AUTO_TEST_SUITE_END()
		BOOST_FIXTURE_TEST_SUITE(can_erase_data_in_iterator_pos, FilledStringList)
			BOOST_AUTO_TEST_CASE(do_not_erase_end_iter_pos)
			{
				BOOST_REQUIRE_THROW(list.Erase(list.end()), std::invalid_argument);
			}
			...
			BOOST_AUTO_TEST_CASE(erase_first_element_change_first_element_to_next)
			{
				auto it = list.begin();
				BOOST_CHECK_EQUAL(*it, "Hello, ");
				BOOST_REQUIRE_NO_THROW(it = list.Erase(it));
			...
			{
				CMyList<std::string> emptyList;
				BOOST_REQUIRE_THROW(emptyList.Erase(emptyList.begin()), std::invalid_argument);
				BOOST_REQUIRE_THROW(emptyList.Erase(emptyList.end()), std::invalid_argument);
			}
		...
		BOOST_AUTO_TEST_SUITE(assignment_operator)
			...
			BOOST_AUTO_TEST_CASE(can_assign_list_after_clean_it)
			{
				...
				copy.Clear();
				list = copy;
				BOOST_CHECK(copy.begin() == list.begin());
				BOOST_CHECK_EQUAL(copy.GetSize(), list.GetSize());
			}
		BOOST_AUTO_TEST_SUITE_END()
		BOOST_AUTO_TEST_SUITE(my_list_construction_tests)
			...
			BOOST_AUTO_TEST_SUITE_END()
		BOOST_AUTO_TEST_SUITE_END()
		BOOST_AUTO_TEST_SUITE(my_list_assignment_operator_exception_tests)
			struct MockObjWithExceptAfterSecondCopy
			{
				MockObjWithExceptAfterSecondCopy() = default;
				MockObjWithExceptAfterSecondCopy(MockObjWithExceptAfterSecondCopy const & other)
				{
					if (other.copyNumber == 1)
					{
			...
			struct MyListContainsMockObj
			{
				MyListContainsMockObj()
				{
					myList.Append(MockObjWithExceptAfterSecondCopy());
					myList.Append(MockObjWithExceptAfterSecondCopy());
					myList.Append(MockObjWithExceptAfterSecondCopy());
				}
				CMyList<MockObjWithExceptAfterSecondCopy> myList;
			};
			BOOST_FIXTURE_TEST_SUITE(after_contains_my_list_mock_obj_throws_except_after_second_copy, MyListContainsMockObj)

				BOOST_AUTO_TEST_CASE(if_assignment_operator_throw_except_my_list_do_not_change)
				{
					BOOST_CHECK_EQUAL(myList.GetSize(), 3);
					CMyList<MockObjWithExceptAfterSecondCopy> tempList;
					tempList.Append(MockObjWithExceptAfterSecondCopy());

					auto begin = myList.begin();
					BOOST_REQUIRE_THROW(myList = tempList, exception);
					BOOST_CHECK_EQUAL(myList.GetSize(), 3);
					BOOST_CHECK(begin == myList.begin());
